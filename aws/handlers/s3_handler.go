...
// S3Handler is an interface for S3 handlers.
type S3Handler interface {
	ListBucketObjects(c *gin.Context)
	GetBucketObject(c *gin.Context)
}
...
// NewS3Handler creates and returns a new instance of S3Handler.
...
func (h *s3HandlerImpl) GetBucketObject(c *gin.Context) {
	bucketName := c.Param("bucketName")
	...
	if err != nil {
		// You can add more logic to check if the error is NotFound
